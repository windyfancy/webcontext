...
module.exports = class Session extends Emitter{
    ...
    static cleanSchedule(database){
        database.query("select token from session_memory where s_key='lass_access_time' and TIMESTAMPDIFF(MINUTE,from_unixtime(s_value),now())>1 limit 50",[]).then((result)=>{
            var arr=[];
            result.forEach((item)=>{
                arr.push(item["token"])
            })
